...
public class SysCodeValueServiceImpl extends BaseServiceImpl<SysCodeValue> implements SysCodeValueService {
    ...
    public Page<SysCodeValue> list(SysCodeValueVo valueVo) {
        Specification<SysCodeValue> specification = Specifications.<SysCodeValue>and()
                .eq(StringUtils.isNotBlank(valueVo.getCodeType()), "codeType", valueVo.getCodeType())
                .like(StringUtils.isNotBlank(valueVo.getName()), "name", "%" + valueVo.getName() + "%")
                .like(StringUtils.isNotBlank(valueVo.getValue()), "value", "%" + valueVo.getName() + "%")
                .like(StringUtils.isNotBlank(valueVo.getRemark()), "remark", "%" + valueVo.getName() + "%")
                .build();
